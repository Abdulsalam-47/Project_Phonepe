...
if select == "HOME":
    ...
    with col2:
        st.video(r"D:\Guvi Projects\Phonepe\Phone Pe Ad.mp4")

    col3,col4= st.columns(2)
    
    with col3:
        st.image(Image.open(r"D:\Guvi Projects\Phonepe\phonepe img3.jpg"),width=500)
    ...
    with col6:
        st.video(r"D:\Guvi Projects\Phonepe\PhonePe Motion Graphics.mp4")
    ...
    with tab1:
        ...
        aggre_method = st.selectbox("Select The Method", ["Aggregated Insurance Analysis", "Aggregated Transaction Analysis", "Aggregated User Analysis"])
       
        if aggre_method == "Aggregated Insurance Analysis":
            
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Aggregated Insurance Analysis", options=aggregate_insurance["Years"].unique())
            Aggre_insur_tac_Y = Transaction_amount_count_Y(aggregate_insurance, years)
            
            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Aggregated Insurance Analysis", options=Aggre_insur_tac_Y["Quarter"].unique())
            Transaction_amount_count_Q(Aggre_insur_tac_Y, quarters)
    
        elif aggre_method == "Aggregated Transaction Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Aggregated Transaction Analysis", options=aggregate_transactions["Years"].unique())
            Aggre_trans_tac_Y = Transaction_amount_count_Y(aggregate_transactions, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Aggregated Transaction Analysis", options=Aggre_trans_tac_Y["States"].unique())
            Aggre_trans_type(Aggre_trans_tac_Y, states)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Aggregated Transaction Analysis", options=Aggre_trans_tac_Y["Quarter"].unique())
            Aggre_trans_tac_Y_Q = Transaction_amount_count_Q(Aggre_trans_tac_Y, quarters)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Aggregated Transaction Analysis for a Quarter", options=Aggre_trans_tac_Y_Q["States"].unique())
            Aggre_trans_type(Aggre_trans_tac_Y_Q, states)
       
        elif aggre_method == "Aggregated User Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Aggregated User Analysis", options=aggregate_users["Years"].unique())
            Aggre_user_Y = Aggre_user_plot_1(aggregate_users, years)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Aggregated User Analysis", options=Aggre_user_Y["Quarter"].unique())
            Aggre_user_y_q = Aggre_user_plot_2(Aggre_user_Y, quarters)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Aggregated User Analysis", options=Aggre_user_y_q["States"].unique())
            Aggre_user_plot_3(Aggre_user_y_q, states)

    with tab2:

        map_method = st.selectbox("Select The Method", ["Map Insurance Analysis", "Map Transaction Analysis", "Map User Analysis"])

        if map_method == "Map Insurance Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Map Insurance Analysis", options=map_insurance["Years"].unique())
            map_insur_tac_Y = Transaction_amount_count_Y(map_insurance, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Map Insurance Analysis", options=map_insur_tac_Y["States"].unique())
            map_insur_district = map_dist(map_insur_tac_Y, states)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Map Insurance Analysis", options=map_insur_tac_Y["Quarter"].unique())
            map_insur_tac_Y_Q = Transaction_amount_count_Q(map_insur_tac_Y, quarters)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Map Insurance Analysis for a Quarter", options=map_insur_tac_Y_Q["States"].unique())
            map_dist(map_insur_tac_Y_Q, states)

        elif map_method == "Map Transaction Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Map Transaction Analysis", options=map_transactions["Years"].unique())
            map_trans_tac_Y = Transaction_amount_count_Y(map_transactions, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Map Transaction Analysis", options=map_trans_tac_Y["States"].unique())
            map_trans_district = map_dist(map_trans_tac_Y, states)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Map Transaction Analysis", options=map_trans_tac_Y["Quarter"].unique())
            map_trans_tac_Y_Q = Transaction_amount_count_Q(map_trans_tac_Y, quarters)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Map Transaction Analysis for a Quarter", options=map_trans_tac_Y_Q["States"].unique())
            map_dist(map_trans_tac_Y_Q, states)

        elif map_method == "Map User Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Map User Analysis", options=map_users["Years"].unique())
            map_user_Y = map_user_plot_1(map_users, years)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Map User Analysis", options=map_user_Y["Quarter"].unique())
            map_user_Y_Q = map_user_plot_2(map_user_Y, quarters)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Map User Analysis", options=map_user_Y_Q["States"].unique())
            map_user_plot_3(map_user_Y_Q, states)

    with tab3:

        top_method = st.selectbox("Select The Method", ["Top Insurance Analysis", "Top Transaction Analysis", "Top User Analysis"])

        if top_method == "Top Insurance Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Top Insurance Analysis", options=top_insurance["Years"].unique())
            top_insur_tac_Y = Transaction_amount_count_Y(top_insurance, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Top Insurance Analysis", options=top_insur_tac_Y["States"].unique())
            top_plt_1(top_insur_tac_Y, states)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Top Insurance Analysis", options=top_insur_tac_Y["Quarter"].unique())
            top_insur_Y_Q = Transaction_amount_count_Q(top_insur_tac_Y, quarters)

        elif top_method == "Top Transaction Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Top Transaction Analysis", options=top_transactions["Years"].unique())
            top_trans_tac_Y = Transaction_amount_count_Y(top_transactions, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Top Transaction Analysis", options=top_trans_tac_Y["States"].unique())
            top_plt_1(top_trans_tac_Y, states)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("Select The Quarter for Top Transaction Analysis", options=top_trans_tac_Y["Quarter"].unique())
            top_trans_Y_Q = Transaction_amount_count_Q(top_trans_tac_Y, quarters)

        elif top_method == "Top User Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Top User Analysis", options=top_users["Years"].unique())
            top_user_Y = top_user_plt_1(top_users, years)

            col1, col2 = st.columns(2)
            with col1:
                states = st.selectbox("Select the State for Top User Analysis", options=top_user_Y["States"].unique())
